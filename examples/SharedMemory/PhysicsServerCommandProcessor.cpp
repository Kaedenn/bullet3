...
#ifdef B3_ENABLE_TINY_AUDIO
	b3Printf("TinyAudio sound engine enabled");
...
bool PhysicsServerCommandProcessor::processUserDebugDrawCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes)
{
	...
	/* Kaedenn 2019/09/10 */
	if (clientCmd.m_updateFlags & USER_DEBUG_ADD_BUTTON)
	{
	...
	/* Kaedenn 2019/09/10 */
	if (clientCmd.m_updateFlags & USER_DEBUG_READ_BUTTON)
	{
		int ok = m_data->m_guiHelper->readUserDebugButton(
			clientCmd.m_userDebugDrawArgs.m_itemUniqueId,
			&serverCmd.m_userDebugDrawArgs.m_parameterValue);
		if (ok)
		{
	...
	/* Kaedenn 2019/10/17 */
	if (clientCmd.m_updateFlags & USER_DEBUG_RESET_BUTTON)
	{
		int ok = m_data->m_guiHelper->resetUserDebugButton(
			clientCmd.m_userDebugDrawArgs.m_itemUniqueId);
		if (ok)
		{
			serverCmd.m_type = CMD_USER_DEBUG_DRAW_COMPLETED;
...
bool PhysicsServerCommandProcessor::processSendPhysicsParametersCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes)
{
	...
	if (clientCmd.m_updateFlags & SIM_PARAM_UPDATE_GRAVITY)
	{
		...
		if (m_data->m_dynamicsWorld->getGravity() != grav) {
			m_data->m_dynamicsWorld->setGravity(grav);
#ifndef SKIP_SOFT_BODY_MULTI_BODY_DYNAMICS_WORLD
	...
	/* Kaedenn 2019/10/13 */
	if (clientCmd.m_updateFlags & SIM_PARAM_UPDATE_VERBOSE_MODE)
	{
		m_data->m_verboseOutput = clientCmd.m_physSimParamArgs.m_verboseMode;
		b3Printf("Setting verbose mode to %d for all available objects", m_data->m_verboseOutput);
	}
